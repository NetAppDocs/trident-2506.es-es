...
Ahora puedes crear el `TridentOrchestrator` e instalar Trident. Opcionalmente, puedes ampliar aún más link:../trident-install/kubernetes-customize-deploy.html["Personaliza tu instalación de Trident"] usando los atributos en la `TridentOrchestrator` spec. El siguiente ejemplo muestra una instalación donde las imágenes de Trident y CSI están ubicadas en diferentes registros.
