...
NOTE: Si el instalador no se completa correctamente o `trident-controller-<generated id>` (`trident-csi-<generated id>` en versiones anteriores a 23.01) no tiene un estado *Running*, la plataforma no se instaló. Usa `-d` para link:../troubleshooting.html#unsuccessful-trident-deployment-using-tridentctl["Activar el modo de depuración"] y solucionar el problema.
...
[%collapsible]
====
Para obtener más información sobre «force detach», consulta link:kubernetes-customize-deploy.html["Personalizar la instalación del operador Trident"].
