...
summary: Puede hacer que Trident Protect reconozca una aplicación que desea administrar creando un CR de aplicación y un CR de AppVault asociado. 
---
...
Debe crear un objeto de configuración (CR) de AppVault que se utilizará al realizar operaciones de protección de datos en la aplicación, y dicho objeto de configuración debe residir en el clúster donde está instalado Trident Protect.  La solicitud de cambio (CR) de AppVault es específica de su entorno; para ver ejemplos de solicitudes de cambio de cambio de AppVault, consultelink:trident-protect-appvault-custom-resources.html["Recursos personalizados de AppVault."]
