...
summary: Personaliza la configuración predeterminada de Trident Protect para adaptarla a tus necesidades específicas. 
---
...
[role="lead"]
Puede personalizar la configuración predeterminada de Trident Protect para adaptarla a los requisitos específicos de su entorno.



== Especifique los límites de recursos del contenedor de Trident Protect.

Puede utilizar un archivo de configuración para especificar los límites de recursos para los contenedores de Trident Protect después de instalar Trident Protect.  Establecer límites de recursos le permite controlar cuántos recursos del clúster consumen las operaciones de Trident Protect.

.Pasos
. Crea un archivo llamado `resourceLimits.yaml` .
. Complete el archivo con las opciones de límite de recursos para los contenedores de Trident Protect según las necesidades de su entorno.
+
...
jobResources:
  ...
  kopiaVolumeRestore:
    ...
    requests:
      ...
      ephemeralStorage: ""
----
. Aplique los valores de `resourceLimits.yaml` archivo:
+
[source, console]
----
...
Puede utilizar un archivo de configuración para modificar las restricciones del contexto de seguridad (SCC) de OpenShift para los contenedores de Trident Protect después de instalar Trident Protect.  Estas restricciones definen las limitaciones de seguridad para los pods en un clúster de Red Hat OpenShift.
...
| crear | Determina si se puede crear un recurso SCC.  Un recurso SCC se creará solo si `scc.create` está configurado para `true` y el proceso de instalación de Helm identifica un entorno OpenShift.  Si no se está operando en OpenShift, o si `scc.create` está configurado para `false` , no se creará ningún recurso SCC. | verdadero 
...
|===
. Aplique los valores de `sccconfig.yaml` archivo:
+
[source, console]
----
...
| autoSupport.inseguro | booleano | Omite la verificación TLS para las conexiones proxy de AutoSupport cuando está configurado en `true` . Úselo sólo para conexiones proxy inseguras.  (por defecto: `false` ) 


| autoSupport.enabled | booleano | Habilita o deshabilita las cargas diarias del paquete AutoSupport de Trident Protect. Cuando se configura para `false` Las cargas diarias programadas están desactivadas, pero aún puede generar manualmente paquetes de soporte.  (por defecto: `true` ) 
...
----
. Después de rellenar el `values.yaml` Archivo con los valores correctos, aplicar el archivo de configuración:
+
[source, console]
----
...
== Restringir los pods de protección de Trident a nodos específicos

Puedes usar la restricción de selección de nodos nodeSelector de Kubernetes para controlar cuáles de tus nodos son elegibles para ejecutar pods de Trident Protect, según las etiquetas de los nodos.  Por defecto, Trident Protect está restringido a nodos que ejecutan Linux.  Puedes personalizar aún más estas restricciones según tus necesidades.

.Pasos
. Crea un archivo llamado `nodeSelectorConfig.yaml` .
. Agregue la opción nodeSelector al archivo y modifique el archivo para agregar o cambiar las etiquetas de los nodos para restringirlos según las necesidades de su entorno.  Por ejemplo, el siguiente archivo contiene la restricción predeterminada del sistema operativo, pero también se dirige a una región y un nombre de aplicación específicos:
+
[source, yaml]
----
nodeSelector:
...
----
. Aplique los valores de `nodeSelectorConfig.yaml` archivo:
+
[source, console]
----
